...
export default function ProblemCard({ problem, onShowSolution }: Readonly<ProblemCardProps>) {
    return <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-slate-900/80 p-6 shadow-xl shadow-slate-950/20 transition hover:-translate-y-1 hover:border-sky-500/30 hover:bg-slate-900">
            <div className="mb-4 flex items-start justify-between gap-4">
                <div>
                    <p className="text-sm font-semibold uppercase tracking-[0.18em] text-sky-300">#{problem.number}</p>
                ...
                <div className="space-y-2 text-right text-sm text-slate-400">
                    <span className="inline-flex items-center gap-2 rounded-full bg-slate-800/70 px-3 py-1">
                        ...
                        {problem.runtime != null ? `${problem.runtime} ms` : 'N/A'}
                    </span>
                    <span className="inline-flex items-center gap-2 rounded-full bg-slate-800/70 px-3 py-1">
                        <Cpu className="size-4" />
                        {problem.memory != null ? `${problem.memory} MB` : 'N/A'}
                    </span>
                </div>
            </div>
            <p className="mb-6 min-h-[5rem] text-sm leading-6 text-slate-300 line-clamp-4">{problem.description}</p>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <button
                    type="button"
                    onClick={() => onShowSolution(problem.solution, problem.title, problem.runtime, problem.memory)}
